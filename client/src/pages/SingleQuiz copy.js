...
import { checkExist, generateRandomTest } from '../utils/helpers';
...
export default function SinglequizPage() {
    ...
    const recentFlag = checkExist(recentAttempt.grade);
    // const activateFlag = recentAttempt.grade ? false : true;
    ...
    const [grade, setGrade] = useState(recentAttempt.grade || 'N/A');
    const [timer, setTimer] = useState(recentAttempt.rawScore || 0);
    ...
    // if (recentAttempt.grade) {
    //     setAllowQuiz(false);
    //     setActivateQuiz(false);
    //     setGrade(recentAttempt.grade);
    //     setRawScore(recentAttempt.rawScore);
    // }
    ...
    //     return generateRandomTest(rawQuestions);
    // }, []);


    // useEffect(() => {
    //     const newQuestions = generateRandomTest(rawQuestions);
            ...
            <ThemeProvider theme={theme}>
                <Container maxWidth="lg" id="quizPanel" >
                    ...
                    <Typography variant="h5" align='center' pt={3} fontStyle={'italic'} color='primary.main' >
                        ...
                        {generateRandomTest(questions).map((question, index) => (
                            <>
                            <div key={index}>
